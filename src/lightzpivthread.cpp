...
void CLightWorker::ThreadLightZPIVSimplified() {
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "xp-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().Zerocoin_Block_V2_Start()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZXPSimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "xp-light-thread", genWit.getPfrom()->addrName);
