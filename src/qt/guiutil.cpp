...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter an eXperience Points address (e.g. %1)").arg("PQ72SjLCNR1HCwsXRrEJWNgAkAFe11sWTx"));
...
{
    // return if URI is not valid or is no xp: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert xp:// to xp:
    //
    //    Cannot handle this later, because xp:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open eXperiencePoints.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "eXperiencePoints.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for eXperiencePoints.lnk
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a eXperiencePoints.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=eXperiencePoints\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the eXperiencePoints app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add eXperiencePoints app to startup item list
